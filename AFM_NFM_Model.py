# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 15:36:31 2018
...
class AFM_NFM(BaseEstimator, TransformerMixin):
    
    def __init__(self, features_M, em_factor, attention_factor,layers, epoch, batch_size, learning_rate,
                 keep_prob, optimizer_type, bn, activation_function, verbose, early_stop,
                 attention, fields, lamda_attention, lamda_em, lamda_layers,decay, save_file, random_seed=2016):
        ...
        lamda_layers: 对各隐藏层的参数使用正则化的系数，0则不使用
        decay：BN操作中移动平均系数
        ...
        self.lamda_em = lamda_em
        self.lamda_layers = lamda_layers
        self.decay = decay
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # 默认使用cpu:
            ...
            if self.attention:
                ...
                self.attention_exp = tf.exp(tf.reduce_sum(tf.multiply(self.weights['attention_p'], self.activation_function(self.attention_mul + \
            ...
            # ___________ 深度网络层_________________
            for i in range(0, len(self.layers)):
                self.AFM = tf.add(tf.matmul(self.AFM, self.weights['layer_%d' %(i+1)]), self.weights['bias_%d' %(i+1)]) # None * layers[i]
                if self.bn:
                    self.NAFM = self.batch_norm_layer(self.AFM, train_phase=self.train_phase, scope_bn='bn_%d' %(i+1))
                self.AFM = self.activation_function(self.AFM)
                self.AFM = tf.nn.dropout(self.AFM, self.dropout_keep[i+1])
            ...
            if self.lamda_layers > 0:
                for i in range(0, len(self.layers)):
                    self.loss = self.loss + tf.contrib.layers.l2_regularizer(self.lamda_layers)(self.weights['layer_%d' %(i+1)])  # regulizer       
    ...
    def _initialize_weights(self):
        ...
        if num_layer > 0:
            glorot = np.sqrt(2.0 / (self.em_factor + self.layers[0]))
            all_weights['layer_1'] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(self.em_factor, self.layers[0])), dtype=np.float32)
            all_weights['bias_1'] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(1, self.layers[0])), dtype=np.float32)
        
        for i in range(1, num_layer):
            glorot = np.sqrt(2.0 / (self.layers[i-1] + self.layers[i+1]))
            all_weights['layer_%d' %(i+1)] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(self.layers[i-1], self.layers[i])), dtype=np.float32)
            all_weights['bias_%d' %(i+1)] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(1, self.layers[i])), dtype=np.float32)            
    ...
    def evaluate(self, data):  # 评价给定的数据集
        ...
        while len(batch_xs['X']) > 0:
            ...
            feed_dict = {self.train_features: batch_xs['X'], self.train_labels: [[y] for y in batch_xs['Y']], self.dropout_keep: np.array([1 for i in range(0,len(self.keep_prob))]), self.train_phase: False}
